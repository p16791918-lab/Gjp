...
SECTIONS = [
    ('Ⅰ. 에너지대사 — 매년 다수 출제', [
        ...
        {
            'title': '글리코겐 대사 · 당원병', 'stars': '★★ 기본 · 당원병 감별',
            'core': [
                '글리코겐 합성은 <b>글리코겐 합성효소</b>(인슐린이 활성)가, 분해는 <b>글리코겐 인산화효소</b>'
                '(글루카곤·에피네프린이 활성)가 속도조절한다.',
                '<b>간 글리코겐</b>은 혈당 유지용으로 포도당-6-인산가수분해효소(G6Pase)로 포도당을 내보내고, '
                '<b>근육 글리코겐</b>은 이 효소가 없어 자기 수축에만 쓰고 혈당에는 직접 기여하지 못한다.',
                '당원병: <b>폰기르케(제I형·G6Pase 결핍)</b>는 공복 저혈당·간비대, <b>폼페(제II형·리소좀 산성말타아제)</b>는 '
                '심장비대, <b>맥아들(제V형·근육 인산화효소)</b>은 운동 시 근경련·근육통(정맥혈 젖산 안 오름)이 특징이다.'],
            'table': None,
            'img': None,
            'trap': '⚠ 함정 — 근육 글리코겐이 혈당을 직접 올린다고 낚는다(G6Pase 없어 못 함). 폰기르케(간·저혈당)와 '
                    '폼페(리소좀·심장)를 바꾼다.'},
    ]),
    ('Ⅱ. 산화스트레스 · 산소운반', [
        {
    ...
    ('Ⅲ. 아미노산 · 질소대사', [
        ...
        {
            'title': '헴 합성 · 분해 · 빌리루빈(황달)', 'stars': '★★ 기본 · 황달 감별',
            'core': [
                '헴 합성은 미토콘드리아에서 <b>ALA 합성효소</b>(속도조절, 헴이 음성되먹임)가 글리신+숙시닐CoA로 '
                '시작하며, 납·철결핍이 이 경로를 방해하고 포르피린증은 중간효소 결핍으로 포르피린이 쌓여 광과민·복통·'
                '신경증상을 낸다.',
                '헴 분해: 늙은 적혈구의 헴 → 빌리베르딘 → <b>비결합(간접) 빌리루빈</b>(지용성·알부민 운반) → 간에서 '
                '<b>글루쿠론산 포합 → 결합(직접) 빌리루빈</b> → 담즙으로 배설된다.',
                '황달은 <b>용혈=간접 빌리루빈↑</b>, <b>담도 폐쇄=직접 빌리루빈↑</b>, 신생아 황달·길버트증후군은 포합효소'
                '(UGT) 미성숙·부족으로 간접 빌리루빈이 오른다.'],
            'table': {'header': ['상황', '우세 빌리루빈'],
                      'rows': [
                          ['용혈', '간접(비결합)↑'],
                          ['간세포 손상', '간접+직접 혼합'],
                          ['담도 폐쇄', '직접(결합)↑'],
                          ['신생아·길버트', '간접(UGT↓)']]},
            'img': None,
            'trap': '⚠ 함정 — 담도폐쇄를 간접빌리루빈↑로 낚는다(직접↑). 간접(비결합) 빌리루빈은 지용성·알부민 결합이라 '
                    '소변으로 안 나간다.'},
        {
            'title': '뉴클레오타이드 합성 · 관련 약물', 'stars': '★★ 기본 · 항암제 표적',
            'core': [
                '퓨린은 <b>PRPP 위에서 처음부터(de novo)</b> 조립해 IMP를 거쳐 AMP·GMP가 되고, 재활용(salvage)은 '
                'HGPRT가 담당한다(레시-나이한=HGPRT 결핍).',
                '피리미딘은 반대로 <b>고리를 먼저 만든 뒤 PRPP에 붙여</b> UMP를 거친다.',
                '항암·면역억제제가 이 경로를 막는다: <b>메토트렉세이트(DHFR·엽산)</b>, <b>5-FU(티미딜산 합성효소)</b>, '
                '하이드록시유레아(리보뉴클레오타이드 환원효소), 6-머캅토퓨린(퓨린 합성).'],
            'table': None,
            'img': None,
            'trap': '⚠ 함정 — 퓨린 de novo가 완성된 염기를 먼저 만든다고 낚는다(PRPP 위에서 조립). 통풍약 알로퓨리놀'
                    '(잔틴산화효소=분해 억제)과 합성 억제를 혼동한다.'},
    ]),
    ('Ⅳ. 지질 · 콜레스테롤', [
        {
    ...
    ('Ⅴ. 분자생물학 · 세포주기', [
            ...
            'trap': '⚠ 함정 — 인트론 5′ 공여부위(GU) 변이를 캡 형성·폴리A·RNA 편집 이상으로 낚는다(정답은 스플라이싱).'},
        {
            ...
            'trap': '⚠ 함정 — 단일가닥 RNA·비상보 상황에 Chargaff 등식을 그대로 적용하도록 낚는다(A=T·G=C는 '
                    '이중가닥 DNA 규칙).'},
        {
            'title': '세포주기 · 사이클린-CDK · 항암제 표적', 'stars': '★★★ 4년(22·23·24·25)',
            'core': [
                '세포주기는 <b>G1(성장) → S(DNA 복제) → G2 → M(유사분열)</b>으로 돌고, 분열하지 않는 세포는 G0에 '
                '머문다.',
                '진행은 <b>사이클린-CDK</b> 복합체가 몬다 — G1/S는 사이클린 D-CDK4/6와 E-CDK2, S기는 A-CDK2, M기는 '
                'B-CDK1(MPF)이 담당한다.',
                '<b>G1 제한점(R점)</b>에서 사이클린 D-CDK4/6가 <b>RB를 인산화</b>해 E2F를 풀어주면 S기로 넘어가고, '
                '<b>p53</b>은 손상 시 p21로 CDK를 막아 주기를 멈추거나 자멸사를 유도한다(RB·p53은 대표 종양억제유전자).',
                '체크포인트(G1/S·G2/M·방추사)가 손상·미부착을 감지해 진행을 막으며, 항암제는 <b>S기(5-FU·MTX)·M기'
                '(빈크리스틴·탁센)</b>에 작용한다. 유사분열 후기엔 APC/C가 세큐린을 분해해 자매염색분체를 분리한다.'],
            'table': {'header': ['시기', '주요 복합체', '사건'],
                      'rows': [
                          ['G1/S', '사이클린 D-CDK4/6 · E-CDK2', 'RB 인산화 → S기 진입'],
                          ['S', '사이클린 A-CDK2', 'DNA 복제'],
                          ['M', '사이클린 B-CDK1(MPF)', '유사분열'],
                          ['정지', 'p53 → p21', '손상 시 주기 정지·수선']]},
            'img': ('figures/2022_2_사진3.jpg', '▲ 유사분열 중기 — 염색체가 적도판에 정렬 (2022 2교시)'),
            'trap': '⚠ 함정 — 5-FU·MTX(S기)와 빈크리스틴·탁센(M기)의 작용 시기를 바꾼다. RB 인산화를 주기 정지로 '
                    '낚는다(인산화되면 오히려 진행).'},
        {
            'title': '세포자멸사(apoptosis) 경로', 'stars': '★★ 기본 · 병리 연결',
            'core': [
                '자멸사는 예정된 능동적 세포죽음으로, 세포가 쪼그라들어 <b>자멸소체</b>로 정리돼 <b>염증을 남기지 않는다</b>'
                '(터져서 염증을 부르는 괴사와 대비).',
                '<b>내인(미토콘드리아) 경로</b>는 세포 스트레스·DNA손상에서 <b>Bcl-2 억제·Bax↑</b>로 시토크롬 c가 나와 '
                '<b>카스파아제 9</b>를 켜고, p53이 이를 촉진한다.',
                '<b>외인(사멸수용체) 경로</b>는 Fas·TNF 수용체에 리간드가 붙어 <b>카스파아제 8</b>을 켠다.',
                '두 경로 모두 <b>실행 카스파아제(3·6·7)</b>로 모여 단백·DNA를 토막 낸다.'],
            'table': None,
            'img': None,
            'trap': '⚠ 함정 — 자멸사가 염증을 일으킨다고 낚는다(무염증). 내인경로 개시를 카스파아제 8로 낚는다'
                    '(9번; 8번은 외인).'},
        {
            'title': 'DNA 복제 · 수선', 'stars': '★★ 기본 · 질환 연결',
            'core': [
                'DNA는 반보존적으로 복제되며, DNA 중합효소는 <b>5′→3′로만</b> 붙여 선도가닥은 연속, 지연가닥은 '
                '<b>오카자키 절편</b>으로 만든 뒤 라이게이스가 잇는다.',
                '텔로머레이스는 말단 텔로미어를 늘려 노화를 늦추며, 대부분 체세포엔 없지만 <b>암세포에서 재활성</b>된다.',
                '수선기전과 질환: <b>뉴클레오타이드 절제수선(NER)</b> 결함=색소건피증(자외선 피리미딘 이량체), '
                '<b>불일치 수선(MMR)</b> 결함=린치증후군, <b>이중가닥 절단 수선(상동재조합)</b> 결함=BRCA 유방·난소암.'],
            'table': None,
            'img': None,
            'trap': '⚠ 함정 — 색소건피증(NER)과 린치증후군(MMR)의 수선기전을 바꾼다. 중합효소가 3′→5′로 붙인다고 '
                    '낚는다(합성은 5′→3′; 3′→5′는 교정 활성).'},
        {
            'title': '유전 양식(멘델 · 반성 · 미토콘드리아 · 각인)', 'stars': '★★ 기본 · 가계도',
            'core': [
                '상염색체 <b>우성</b>은 매세대 수직으로 나타나고(마르판·헌팅턴), <b>열성</b>은 보인자 부모에서 25% 발현한다'
                '(낭성섬유증·대사질환).',
                '<b>X연관(반성) 열성</b>(혈우병·색맹·뒤셴)은 남자에서 주로 발현하며 <b>아버지→아들 전달이 없다</b>.',
                '<b>미토콘드리아 유전</b>은 <b>모계로만</b> 전달돼 모든 자녀에게 갈 수 있다(레베르시신경병증·MELAS).',
                '<b>각인(imprinting)</b>은 부·모 어느 쪽에서 왔는지에 따라 발현이 달라진다 — 프래더-윌리(부계 결실)·'
                '안젤만(모계 결실)이 대표다.'],
            'table': None,
            'img': None,
            'trap': '⚠ 함정 — 미토콘드리아 유전을 부계전달로 낚는다(모계만). X연관 열성에서 아버지가 아들에게 준다고 '
                    '낚는다(전달 안 됨).'},
    ]),
    ('Ⅵ. 임상표지자 · 효소 · 신호', [
        {
            ...
            'trap': '⚠ 함정 — NO의 이차전령을 cAMP·IP₃로 낚는다(정답은 cGMP). 전구 아미노산은 아르지닌이다.'},
        {
            ...
            'trap': '⚠ 함정 — DKA의 산증을 정상음이온차로 낚거나(고음이온차), 초기 혈청칼륨이 정상·상승이어도 몸 전체 '
                    '칼륨은 부족함을 놓친다.'},
        {
            'title': '세포 신호전달(GPCR · 2차전령 · RTK)', 'stars': '★★ 기본 · 여러 교시 연결',
            'core': [
                'G단백연결수용체(GPCR)는 <b>Gs(cAMP↑)·Gi(cAMP↓)·Gq(PLC→IP₃·DAG→Ca²⁺)</b>로 갈려 2차전령을 만든다.',
                '수용체 <b>티로신인산화효소(RTK)</b>는 인슐린·성장인자 수용체로, 자가인산화 뒤 <b>RAS-MAPK·PI3K</b> 경로를 '
                '켠다.',
                '스테로이드·갑상샘호르몬은 지용성이라 막을 통과해 <b>세포내 수용체</b>에 붙어 유전자 전사를 직접 바꾼다'
                '(2차전령을 쓰지 않음).',
                '2차전령: cAMP(→PKA), cGMP(→NO·나트륨이뇨펩티드), IP₃(→소포체 Ca²⁺ 방출), DAG(→PKC).'],
            'table': {'header': ['G단백', '효과', '대표 수용체'],
                      'rows': [
                          ['Gs', '아데닐산고리화효소↑ · cAMP↑', 'β 아드레날린'],
                          ['Gi', 'cAMP↓', 'M2 · α2'],
                          ['Gq', 'PLC → IP₃·DAG·Ca²⁺', 'α1 · M1·M3']]},
            'img': None,
            'trap': '⚠ 함정 — 스테로이드호르몬을 막수용체·2차전령으로 낚는다(세포내 수용체·직접 전사). Gs와 Gi의 '
                    'cAMP 방향을 바꾼다.'},
        {
            'title': '비타민 · 조효소 총정리', 'stars': '★★ 기본 · 결핍 감별',
            'core': [
                '수용성 비타민은 대부분 조효소로 쓰인다 — B1(TPP·탈탄산), B2(FAD), B3(NAD/NADP), B5(CoA), '
                'B6(PLP·아미노기 전이), B7(비오틴·카복실화), B9·B12(1탄소·메틸 전달).',
                '지용성 비타민 — A(시각·상피), D(칼슘), E(항산화), <b>K(응고인자 II·VII·IX·X의 γ-카복실화)</b>.',
                '대표 결핍 — B1(각기·베르니케), B3(펠라그라 3D: 피부염·설사·치매), B12·엽산(거대적혈모구빈혈, B12만 '
                '신경증상), C(괴혈병·콜라겐 이상), D(구루병/골연화).'],
            'table': {'header': ['비타민', '조효소 / 역할'],
                      'rows': [
                          ['B1', 'TPP(피루브산·α-KG 탈탄산)'],
                          ['B2 · B3', 'FAD / NAD(P)(산화환원)'],
                          ['B6', 'PLP(아미노기 전이)'],
                          ['B9 · B12', '1탄소·메틸 전달(핵산·메티오닌)'],
                          ['K', '응고인자 γ-카복실화']]},
            'img': None,
            'trap': '⚠ 함정 — 비타민K를 항산화로 낚는다(응고인자 카복실화; 항산화는 E). B12·엽산 결핍을 소적혈구로 '
                    '낚는다(거대적혈모구).'},
    ]),
]
