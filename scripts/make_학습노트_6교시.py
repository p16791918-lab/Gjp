    ...
    ('Ⅰ. 약동학 (흡수·분포·대사·배설)', [
                    ...
                    '반감기).'},
        {
            ...
            'core': [
                ...
                '대부분 약물은 농도에 비례해 제거되는 <b>1차(선형) 약동학</b>이다 — 대사효소가 여유로워 농도가 '
                'Km보다 낮으면 미카엘리스-멘텐 식이 거의 직선(제거율 일정)이 되기 때문이다.',
                '반면 에탄올·페니토인·아스피린(고용량)은 농도가 높아 <b>대사효소가 Vmax로 포화</b>되므로, 농도와 '
                '무관하게 <b>단위시간당 일정량</b>만 제거되는 <b>0차 약동학</b>을 보인다(그래서 조금만 더 마셔도 혈중 '
                '농도가 급등).',
    ...
    ('Ⅵ. 내분비 · 항감염 · 항암 · 임상시험', [
                    ...
                    '낚는다(에르고스테롤).'},
        {
            ...
            'trap': '⚠ 함정 — 1상 대상을 환자로 낚는다(건강한 지원자). 특이체질반응을 용량의존으로 낚는다(용량 '
                    '무관).'},
        {
            'title': '기타 표적약 — 위산(PPI) · 갑상샘 · 알코올(ALDH)', 'stars': '★★ 2년(21·23 등)',
            'core': [
                '양성자펌프억제제(오메프라졸 등 -프라졸)는 위벽세포의 <b>H+/K+-ATPase를 비가역 억제</b>해 위산을 '
                '강력히 줄이며, 산성에서 활성화되는 전구약이라 식전 복용한다.',
                '갑상샘저하에는 <b>레보티록신(T4)</b>을 보충하고, 갑상샘항진에는 <b>메티마졸(임신 1기엔 PTU)</b>로 '
                '호르몬 합성을 막는다.',
                '알코올은 알코올탈수소효소로 아세트알데하이드가 됐다가 <b>ALDH2</b>로 아세트산이 되는데, ALDH2가 '
                '약한(아시아인 결핍) 사람은 아세트알데하이드가 쌓여 홍조·두근거림이 난다.',
                '디설피람은 이 ALDH2를 막아 음주 시 불쾌반응을 일으켜 금주를 돕는다.'],
            'table': None,
            'img': None,
            'trap': '⚠ 함정 — PPI 표적을 H2 수용체로 낚는다(H+/K+-ATPase). 술 홍조의 원인을 알코올 자체로 낚는다'
                    '(아세트알데하이드 축적·ALDH2).'},
        {
            'title': '자가작용물질 — 히스타민 · 세로토닌 · 프로스타글란딘', 'stars': '★★ 기본 · 약물표적',
            'core': [
                '<b>히스타민</b>은 비만세포에서 나와 <b>H1(알레르기·기관지수축·가려움)·H2(위산분비)</b>에 작용하며 '
                '각각 항히스타민제·H2차단제가 막는다.',
                '<b>세로토닌(5-HT)</b>은 장·혈소판·중추에 작용하며, 5-HT3 차단제(온단세트론)는 항암 구토를, '
                '트립탄(5-HT1)은 편두통을 다룬다.',
                '<b>프로스타글란딘·류코트리엔</b>은 아라키돈산에서 COX·LOX로 만들어져 염증·통증·기관지수축을 매개하고, '
                'NSAID(COX)·류코트리엔 길항제(몬테루카스트)가 표적한다.'],
            'table': None,
            'img': None,
            'trap': '⚠ 함정 — 아스피린이 류코트리엔을 막는다고 낚는다(COX만 억제; 류코트리엔은 LOX 경로). H1(알레르기)과 '
                    'H2(위산)를 바꾼다.'},
        {
            'title': '항암제 분류 · 특이독성', 'stars': '★★ 기본 · 표적·부작용',
            'core': [
                '<b>알킬화제(사이클로포스파마이드)·백금(시스플라틴)</b>은 DNA를 교차연결하고, <b>항대사제(MTX·5-FU·'
                '6-MP)</b>는 핵산 합성을 막는다(S기).',
                '<b>미세소관 약물</b>: 빈크리스틴(중합 억제)·파클리탁셀(탈중합 억제)은 M기 방추사를 표적한다.',
                '토포이소머라제 억제제(독소루비신·에토포시드), 표적치료(트라스투주맙=HER2·이마티닙=BCR-ABL/KIT)도 '
                '있다.',
                '공통 독성은 골수억제·점막염·탈모이며, 특이독성으로 <b>독소루비신=심장, 블레오마이신=폐, 시스플라틴='
                '신장·귀</b>, 사이클로포스파마이드=출혈성 방광염이 있다.'],
            'table': None,
            'img': None,
            'trap': '⚠ 함정 — 항대사제(S기)와 미세소관 약물(M기)의 시기를 바꾼다. 독소루비신 독성을 폐로 낚는다'
                    '(심장; 폐는 블레오마이신).'},
    ]),
]
...
def build():
    doc = SimpleDocTemplate('학습노트_pdf/기종평_6교시_빈출유형_학습노트.pdf', pagesize=A4,
