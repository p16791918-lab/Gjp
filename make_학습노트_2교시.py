...
SECTIONS = [
    ('Ⅰ. 에너지대사 — 매년 다수 출제', [
                    ...
                    '포도당으로 낚지만 케톤체다.'},
        {
            ...
            'core': [
                'TCA 회로는 미토콘드리아 기질에서 일어나며, 아세틸CoA가 옥살아세트산과 합쳐 시트르산으로 들어가고 '
                '한 바퀴에 NADH 3·FADH₂ 1·GTP 1을 낸다(GTP는 숙시닐CoA→숙신산 단계).',
                '에너지가 남으면 시트르산이 세포질로 나가(시트르산 셔틀) 아세틸CoA를 내놓아 <b>지방산 합성</b>의 재료가 된다.',
                '글루타민은 글루탐산을 거쳐 <b>알파케토글루타르산</b>으로 TCA에 보충(anaplerosis)되므로, 표지된 글루타민을 '
                '넣으면 알파케토글루타르산이 가장 먼저 표지된다.'],
            'table': None,
            'img': None,
            'trap': '⚠ 함정 — GTP 생성 단계(숙시닐CoA→숙신산)를 다른 단계로 낚거나, 글루타민이 TCA로 들어가는 '
                    '진입점을 시트르산·옥살아세트산으로 낚는다(정답은 알파케토글루타르산).'},
        {
...
def build():
    doc = SimpleDocTemplate('기종평_2교시_빈출유형_학습노트.pdf', pagesize=A4,
